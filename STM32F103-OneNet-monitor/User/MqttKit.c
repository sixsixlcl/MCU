								...
								int16 send_len, int8 *type_bin_head, MQTT_PACKET_STRUCTURE *mqttPacket)
{
	...
	if(MQTT_PacketPublish(MQTT_PUBLISH_ID, topic_buf, NULL, send_len, MQTT_QOS_LEVEL1, 0, 1, mqttPacket) == 0)
	{
		/* The caller appends the binary payload after this packet header. */
	}
	else
		return 1;
